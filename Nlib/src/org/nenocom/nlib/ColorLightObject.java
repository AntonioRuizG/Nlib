...
import org.nenocom.nlib.shaders.PerPixelShader;
 ...
 *
 */
public abstract class ColorLightObject {
	...
	public ColorLightObject(NlibRenderer renderer) {
		this.renderer = renderer;
		float[] vertices = getVertices();
		vertexCount = vertices.length/3;
		float[] colors = getColors(vertices);
		float[] normals = getNormals(vertices);
		if(vertices.length/3 != colors.length/4 || vertices.length/3 != normals.length/3){
	...
	protected abstract float[] getNormals(float[] vertices);

	protected abstract float[] getColors(float[] vertices);
